"""
    ...
    "###.###.###-##", a verificação do CPF acontece utilizando
     os 9 primeiros dígitos e, com um cálculo simples,
     verificando se o resultado corresponde aos dois últimos dígitos
     (depois do sinal "-").

    Vamos usar como exemplo, um CPF fictício "529.982.247-25".

    Validação do primeiro dígito
    Primeiramente multiplica-se os 9 primeiros dígitos pela
    sequência decrescente de números de 10 à 2 e soma os resultados.
    Assim: 5 * 10 + 2 * 9 + 9 * 8 + 9 * 7 + 8 * 6 + 2 * 5 + 2 * 4 + 4 * 3 + 7 * 2
    ...
    mais o primeiro dígito verificador, e vamos multiplicar esses 10 números pela sequencia
    ...
    No nosso algoritmo, vamos verificar se todos os dígitos do CPF são iguais e, neste caso,
    considerar que ele é inválido.
...
numeros = []
posation = []
#cpf = str(input('Digite seu CPF sem símbolos: ')).strip()
for p, caracter in enumerate(cpf):
    try:
        numeros.insert(0, int(caracter))
    except:
        posation.append(p)
        posation.append(caracter)

vezes = 0
for repeat in numeros:
    if numeros[0] == repeat:
        vezes += 1
...
while inicio > 0:
    soma = 0
    for count, numero in enumerate(numeros[inicio:]):
        mult = numero * (count + 2)
        soma += mult
    inicio -= 1
    resto = (soma * 10) % 11
